...
if(isset($_POST["si"]) && !empty($_POST["si"])){
    ...
    if(empty($author_name_err) && empty($other_author_err) && empty($paper_title_err) && empty($journal_name_err) && empty($reference_format_err) && empty($paper_link_err) && empty($category_err) && empty( $paper_index_err) && empty($key_words_err)){
        ...
        $sql = "UPDATE tb_publication SET author_name=?, other_author=?, paper_title=?, journal_name=?, reference_format=?, paper_link=?, category=?, paper_index=?, key_words=? WHERE si=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssssi", $param_author_name, $param_other_author, $param_paper_title, $param_journal_name,$param_reference_format, $param_paper_link, $param_category, $param_paper_index, $param_key_words, $param_si);
